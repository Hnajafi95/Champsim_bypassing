...
/*
	if(cache->reuseHelper.size() != 0)
	{
		for(const auto& elem : cache->reuseHelper)
		{
			int reuse_distance = -1;
			if(elem.second->bypass_algorithm == 1)
			{
				if(elem.second->bypass == 1)
				{
					cache->accurate++;
				}
				else
...
//	}
/*
	if(cache->offsets.size() != 0)
        {
                for(uint64_t sig : cache->offsets)
                {
                        if(cache->offset_reuse.find(sig) != cache->offset_reuse.end())
                        {
                        	for(uint64_t reuse : cache->offset_reuse[sig])
				{
					ofstream outFile;
                        		outFile.open("/home/cc/champsim/offset_reuse.csv", std::ios::app);
                        		outFile << " Offset: " << sig
...
/*
        if(cache->offsets.size() != 0)
        {
                for(uint64_t sig : cache->offsets)
                {
                        ...
                        if(cache->offset_hit.find(sig) != cache->offset_hit.end())
                        {
                                hit_count = cache->offset_hit[sig];
                        }

                        if(cache->offset_miss.find(sig) != cache->offset_miss.end())
                        {
                                miss_count = cache->offset_miss[sig];
                        }
        ...
        {
                for(uint64_t sig : cache->pages)
                {
                        uint64_t hit_count = 0;
                        uint64_t miss_count = 0;
                        uint64_t access_count = 0;
                        if(cache->page_hit.find(sig) != cache->page_hit.end())
                        {
                                hit_count = cache->page_hit[sig];
                        }

                        if(cache->page_miss.find(sig) != cache->page_miss.end())
                        {
                                miss_count = cache->page_miss[sig];
                        }
                        access_count = hit_count + miss_count;
        ...
        if(cache->pcs.size() != 0)
	{
		for(uint64_t sig : cache->pcs)
		{
			uint64_t hit_count = 0;
			uint64_t miss_count = 0;
			uint64_t access_count = 0;
			if(cache->pc_hit.find(sig) != cache->pc_hit.end())
			{	
				hit_count = cache->pc_hit[sig];		
			}

			if(cache->pc_miss.find(sig) != cache->pc_miss.end())
			{
				miss_count = cache->pc_miss[sig];
			}
			access_count = hit_count + miss_count;
	...
	if(cache->pc_offset.size() != 0)
	{
                for(uint64_t sig : cache->pc_offset)
                {
                        uint64_t hit_count = 0;
                        uint64_t miss_count = 0;
                        uint64_t access_count = 0;
                        if(cache->pc_offset_hit.find(sig) != cache->pc_offset_hit.end())
                        {
                                hit_count = cache->pc_offset_hit[sig];
                        }

                        if(cache->pc_offset_miss.find(sig) != cache->pc_offset_miss.end())
                        {
                                miss_count = cache->pc_offset_miss[sig];
                        }
                        access_count = hit_count + miss_count;

                        ofstream outFile;
                        outFile.open("/home/cc/champsim/pc_offset.csv", std::ios::app);
                        outFile << " PC_Offset: " << sig
                        << " hit: " << hit_count
                        << " miss: " << miss_count
                        << " access: " << access_count
                        << endl;
	...
	if(cache->pc_page.size() != 0)
	{
                for(uint64_t sig : cache->pc_page)
                {
                        uint64_t hit_count = 0;
                        uint64_t miss_count = 0;
                        uint64_t access_count = 0;
                        if(cache->pc_page_hit.find(sig) != cache->pc_page_hit.end())
                        {
                                hit_count = cache->pc_page_hit[sig];
                        }

                        if(cache->pc_page_miss.find(sig) != cache->pc_page_miss.end())
                        {
                                miss_count = cache->pc_page_miss[sig];
                        }
                        access_count = hit_count + miss_count;
	...
	if(cache->pc_path.size() != 0)
	{
                for(uint64_t sig : cache->pc_path)
                {
                        uint64_t hit_count = 0;
                        uint64_t miss_count = 0;
                        uint64_t access_count = 0;
                        if(cache->pc_path_hit.find(sig) != cache->pc_path_hit.end())
                        {
                                hit_count = cache->pc_path_hit[sig];
                        }

                        if(cache->pc_path_miss.find(sig) != cache->pc_path_miss.end())
                        {
                                miss_count = cache->pc_path_miss[sig];
                        }
                        access_count = hit_count + miss_count;
	...
	if(cache->pc_path_offset.size() != 0)
	{
                for(uint64_t sig : cache->pc_path_offset)
                {
                        uint64_t hit_count = 0;
                        uint64_t miss_count = 0;
                        uint64_t access_count = 0;
                        if(cache->pc_path_offset_hit.find(sig) != cache->pc_path_offset_hit.end())
                        {
                                hit_count = cache->pc_path_offset_hit[sig];
                        }

                        if(cache->pc_path_offset_miss.find(sig) != cache->pc_path_offset_miss.end())
                        {
                                miss_count = cache->pc_path_offset_miss[sig];
                        }
                        access_count = hit_count + miss_count;

                        ofstream outFile;
                        outFile.open("/home/cc/champsim/pc_path_offset.csv", std::ios::app);
                        outFile << " PC_Path_Offset: " << sig
                        << " hit: " << hit_count
                        << " miss: " << miss_count
                        << " access: " << access_count
                        << endl;
	...
	if(cache->pc_path_page.size() != 0)
	{ 
                for(uint64_t sig : cache->pc_path_page)
                {
                        uint64_t hit_count = 0;
                        uint64_t miss_count = 0;
                        uint64_t access_count = 0;
                        if(cache->pc_path_page_hit.find(sig) != cache->pc_path_page_hit.end())
                        {
                                hit_count = cache->pc_path_page_hit[sig];
                        }

                        if(cache->pc_path_page_miss.find(sig) != cache->pc_path_page_miss.end())
                        {
                                miss_count = cache->pc_path_page_miss[sig];
                        }
                        access_count = hit_count + miss_count;

                        ofstream outFile;
                        outFile.open("/home/cc/champsim/pc_path_page.csv", std::ios::app);
                        outFile << " PC_Path_Page: " << sig
                        << " hit: " << hit_count
                        << " miss: " << miss_count
                        << " access: " << access_count
                        << endl;
...
int main(int argc, char** argv)
{
    ...
    uncore.LLC.llc_initialize_bypassing();    
